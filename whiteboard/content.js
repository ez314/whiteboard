...
function home(template) {
    ...
    fetch(urlPrefix+"/learn/api/public/v1/users/" + user_id + "/courses?availability.available=Yes&role=Student&expand=course").then(response => response.json()).then(data => {
        ...
        //return loadAnnouncementCards().then(resp => { return fetchGrades().then(text => { return processAgenda(); }) });

        fetchSidebarCourses().then(data => { return loadAnnouncementCards().then(resp => { return fetchGrades().then(text => { return processAgenda(); }) }); });
        fetchSidebarCourses();
    })
}
...
function fetchSidebarCourses(courseId = "") {
    console.log("fetchSidebarCourses: " + courseId);
